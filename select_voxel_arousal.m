    ...
    for j = 1:80
        for k = 1:46
            if ~any(any(isnan(sdata(:,:,i,j,k))))
                ...
                tempmatrix = tempdata';
                p = anova1(tempmatrix,arousal,"off");
                parousal(i,j,k) = p;
            end
        end
    end
...
save("pvalue-arousal_select-voxel.mat","parousal")

h1 = histogram(parousal(:));
h1.BinWidth = 0.001;
...
while (sum(min_value==1)>0)
    currmin = find(parousal==min(min(min(parousal))));
    [a,b,c] = ind2sub(size(parousal),currmin);
    tempp = parousal(a,b,c);
    parousal(a,b,c) = 1;
...
save("top100voxels_arousal.mat","min_index","min_value","excllist");
...
for i = 1:72
    for j = 1:40
        ...
        train_data(row,101) = arousal(i);
    end
end

% save("train_valence_top-voxel.mat","train_data");
writematrix(train_data,"train_arousal_top-voxel.csv");
...
writematrix(train_data_nonan_both,"train_arousal_top-voxel_nonan_both.csv");
...
for i = 1:72
    for j = 1:30
        ...
        test_data(row,101) = arousal(i);
    end
end

% save("train_valence_top-voxel.mat","train_data");
writematrix(test_data,"test_arousal_top-voxel.csv");
...
writematrix(test_data_nonan_both,"test_arousal_top-voxel_nonan_both.csv");
